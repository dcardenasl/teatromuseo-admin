 ...
 * Defaults:
 *   threshold     = 70
 *   clover-xml    = tests/coverage.xml  (matches phpunit.xml.dist output)
 *
 * Exit codes:
...
$cloverPath = $argv[2] ?? __DIR__ . '/../tests/coverage.xml';

if (! is_readable($cloverPath)) {
    fwrite(STDERR, "check-coverage: clover XML not found at {$cloverPath}\n");
    fwrite(STDERR, "Did you run `phpunit --coverage-clover=tests/coverage.xml` first?\n");
    exit(2);
}
