 ...
 * @internal
 */
final class EventReferenceApiServiceTest extends CIUnitTestCase
{
    public function testListCallsCorrectEndpoint(): void
    {
        ...
        $mock->expects($this->once())
            ->method('get')
            ->with('/events/event-references', [])
            ->willReturn($expected);

        $service = new EventReferenceApiService($mock);
    ...
    public function testGetCallsCorrectEndpoint(): void
    {
        ...
        $mock->expects($this->once())
            ->method('get')
            ->with('/events/event-references/uuid-1')
            ->willReturn($expected);
    ...
    public function testCreateCallsCorrectEndpoint(): void
    {
        ...
        $mock->expects($this->once())
            ->method('post')
            ->with('/events/event-references', $payload)
            ->willReturn($expected);
    ...
    public function testDeleteCallsCorrectEndpoint(): void
    {
        ...
        $mock->expects($this->once())
            ->method('delete')
            ->with('/events/event-references/uuid-3')
            ->willReturn($expected);
