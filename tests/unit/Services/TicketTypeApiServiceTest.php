 ...
 * @internal
 */
final class TicketTypeApiServiceTest extends CIUnitTestCase
{
    public function testListCallsCorrectEndpoint(): void
    {
        ...
        $mock->expects($this->once())
            ->method('get')
            ->with('/events/ticket-types', [])
            ->willReturn($expected);

        $service = new TicketTypeApiService($mock);
    ...
    public function testGetCallsCorrectEndpoint(): void
    {
        ...
        $mock->expects($this->once())
            ->method('get')
            ->with('/events/ticket-types/uuid-1')
            ->willReturn($expected);
    ...
    public function testCreateCallsCorrectEndpoint(): void
    {
        ...
        $mock->expects($this->once())
            ->method('post')
            ->with('/events/ticket-types', $payload)
            ->willReturn($expected);
    ...
    public function testDeleteCallsCorrectEndpoint(): void
    {
        ...
        $mock->expects($this->once())
            ->method('delete')
            ->with('/events/ticket-types/uuid-3')
            ->willReturn($expected);
