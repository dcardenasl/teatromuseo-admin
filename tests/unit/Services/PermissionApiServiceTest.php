 ...
 * @internal
 */
final class PermissionApiServiceTest extends CIUnitTestCase
{
    public function testListCallsCorrectEndpoint(): void
    {
        ...
        $mock->expects($this->once())
            ->method('get')
            ->with('/api/v1/iam/permissions', [])
            ->willReturn($expected);

        $service = new PermissionApiService($mock);
    ...
    public function testGetCallsCorrectEndpoint(): void
    {
        ...
        $mock->expects($this->once())
            ->method('get')
            ->with('/api/v1/iam/permissions/uuid-1')
            ->willReturn($expected);
    ...
    public function testCreateCallsCorrectEndpoint(): void
    {
        ...
        $mock->expects($this->once())
            ->method('post')
            ->with('/api/v1/iam/permissions', $payload)
            ->willReturn($expected);
    ...
    public function testDeleteCallsCorrectEndpoint(): void
    {
        ...
        $mock->expects($this->once())
            ->method('delete')
            ->with('/api/v1/iam/permissions/uuid-3')
            ->willReturn($expected);
