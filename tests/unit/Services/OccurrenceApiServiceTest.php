 ...
 * @internal
 */
final class OccurrenceApiServiceTest extends CIUnitTestCase
{
    public function testListCallsCorrectEndpoint(): void
    {
        ...
        $mock->expects($this->once())
            ->method('get')
            ->with('/events/occurrences', [])
            ->willReturn($expected);

        $service = new OccurrenceApiService($mock);
    ...
    public function testGetCallsCorrectEndpoint(): void
    {
        ...
        $mock->expects($this->once())
            ->method('get')
            ->with('/events/occurrences/uuid-1')
            ->willReturn($expected);
    ...
    public function testCreateCallsCorrectEndpoint(): void
    {
        ...
        $mock->expects($this->once())
            ->method('post')
            ->with('/events/occurrences', $payload)
            ->willReturn($expected);
    ...
    public function testDeleteCallsCorrectEndpoint(): void
    {
        ...
        $mock->expects($this->once())
            ->method('delete')
            ->with('/events/occurrences/uuid-3')
            ->willReturn($expected);
