 ...
 * @internal
 */
final class TicketFlowTest extends CIUnitTestCase
{
    ...
    {
        $result = $this->get('/admin/tickets/tickets');
    ...
    {
        $mock = $this->createMock(TicketApiService::class);
        ...
        Services::injectMock('ticketApiService', $mock);

        $result = $this->withSession([
            'access_token' => 'token',
            'user'         => ['permissions' => ['users.read']],
        ])->post('/admin/tickets/tickets/test-uuid/delete', [
            csrf_token() => csrf_hash(),
        ]);

        $result->assertRedirectTo(site_url('admin/tickets/tickets'));
    }
}
