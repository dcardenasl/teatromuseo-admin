...
<?= view('components/display/admin_page_header', [
    ...
    'title' => 'TicketTypes.ticket_types_edit',
]) ?>

<?php if (has_permission('event.ticket-types.delete')): ?>
    <form id="delete-item-form" method="post" action="<?= route_to('admin.tickettypes.ticket_types.delete', (string) ($item['id'] ?? '')) ?>" x-data @submit.prevent="$store.confirm.show('<?= esc(confirm_delete_message($item['name'] ?? $item['title'] ?? $item['id'] ?? null), 'js') ?>', () => $el.submit())">
        <?= csrf_field() ?>
    </form>
<?php endif; ?>

<form method="post" action="<?= route_to('admin.tickettypes.ticket_types.update', (string) ($item['id'] ?? '')) ?>" class="grid grid-cols-1 gap-6 lg:grid-cols-3">
    <?= csrf_field() ?>

    <div class="lg:col-span-2">
        <section class="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
            <h3 class="text-lg font-semibold text-gray-900"><?= esc(lang('TicketTypes.ticket_types_edit')) ?></h3>
    ...
    <aside class="space-y-6">
        <?= view('components/display/admin_actions_panel', [
            ...
            'dangerContent' => has_permission('event.ticket-types.delete') ? '<button type="submit" form="delete-item-form" class="' . esc(action_button_class('danger'), 'attr') . '">'
