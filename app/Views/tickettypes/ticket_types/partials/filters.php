...
<div class="mt-3 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
    <div class="xl:col-span-2">
        ...
        <input type="text" name="search" value="<?= esc((string) request()->getGet('search')) ?>"
            placeholder="<?= esc(lang('TicketTypes.ticket_types_search_placeholder')) ?>"
            class="<?= esc(filter_input_class()) ?>" data-table-debounce="350">
    </div>
    <div>
        <label class="<?= esc(filter_label_class()) ?>"><?= lang('TicketTypes.field_event_id') ?></label>
    ...
    <div>
        <label class="<?= esc(filter_label_class()) ?>"><?= lang('TicketTypes.field_occurrence_id') ?></label>
