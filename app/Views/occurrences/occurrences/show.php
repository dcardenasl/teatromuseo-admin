...
<?php
    ...
    <?php if (has_permission('event.occurrences.delete')): ?>
        <?php ob_start(); ?>
        <form method="post" action="<?= route_to('admin.occurrences.occurrences.delete', $itemId) ?>" x-data @submit.prevent="$store.confirm.show('<?= esc(confirm_delete_message($occurrenceLabel), 'js') ?>', () => $el.submit())">
            <?= csrf_field() ?>
