...
<div class="border border-dashed border-violet-300 bg-violet-50/20 rounded-xl p-4 <?= $cssClass ?>">
    <div class="flex items-center justify-between mb-3 border-b border-violet-100 pb-2">
        <span class="text-xs font-semibold text-violet-600 uppercase tracking-wider flex items-center gap-1.5">
            ...
            <?= esc(lang('BlockPreview.tabs.title')) ?>
        </span>
        <span class="text-[10px] bg-slate-100 text-slate-700 px-2 py-0.5 rounded-full font-medium"><?= esc(lang('BlockPreview.common.layout')) ?> <?= esc($layout) ?></span>
    </div>
    ...
    <div class="<?= $layout === 'vertical' ? 'flex gap-3' : 'space-y-3' ?>">
        ...
        <div class="<?= $layout === 'vertical' ? 'w-1/4 border-r border-slate-100 pr-2 flex flex-col gap-1' : 'flex gap-1.5 border-b border-slate-100 pb-2 overflow-x-auto' ?>">
            <span class="text-xs bg-primary text-white px-3 py-1.5 rounded-lg text-center font-medium"><?= esc(lang('BlockPreview.tabs.one')) ?></span>
            <span class="text-xs bg-slate-100 text-slate-600 px-3 py-1.5 rounded-lg text-center font-medium"><?= esc(lang('BlockPreview.tabs.two')) ?></span>
        </div>

        <!-- Content Preview -->
        <div class="flex-1 border border-slate-200 bg-white rounded-lg p-3 text-xs text-slate-500">
            <div class="font-bold text-slate-700 mb-1"><?= esc(lang('BlockPreview.tabs.active_content')) ?></div>
            <p><?= esc(lang('BlockPreview.tabs.content')) ?></p>
        </div>
    </div>
    <p class="text-[10px] text-slate-400 mt-3 text-center italic"><?= esc(lang('BlockPreview.tabs.child_hint')) ?></p>
</div>
