...
<div class="<?= $cssClass ?> border-2 border-dashed border-blue-300 rounded-lg p-4">
    <p class="text-xs text-blue-400 text-center mb-3 font-medium uppercase tracking-wide">
        <?= esc(lang('BlockPreview.container.distribution')) ?> <?= esc($layout) ?>
    </p>
    <div class="<?= $innerClass ?>">
        <div class="bg-gray-100 rounded p-4 text-center text-sm text-gray-400"><?= esc(lang('BlockPreview.common.child_one')) ?></div>
        <?php if (in_array($layout, ['grid-2', 'grid-3', 'flex-row'], true)): ?>
            <div class="bg-gray-100 rounded p-4 text-center text-sm text-gray-400"><?= esc(lang('BlockPreview.common.child_two')) ?></div>
        <?php endif; ?>
        <?php if ($layout === 'grid-3'): ?>
            <div class="bg-gray-100 rounded p-4 text-center text-sm text-gray-400"><?= esc(lang('BlockPreview.common.child_three')) ?></div>
        <?php endif; ?>
    </div>
</div>
