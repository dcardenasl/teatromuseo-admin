...
<div class="border border-slate-200 bg-white rounded-lg p-3">
    <div class="text-[10px] font-bold text-violet-500 uppercase mb-1"><?= esc(lang('BlockPreview.accordion.item_label')) ?></div>
    <div class="flex justify-between items-center text-xs font-bold text-slate-800">
        <span><?= $title !== '' ? esc($title) : esc(lang('BlockPreview.accordion.empty_title')) ?></span>
        <span class="text-[9px] px-1.5 py-0.5 rounded font-medium <?= $isOpen ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600' ?>">
            <?= esc(lang($isOpen ? 'BlockPreview.accordion.open' : 'BlockPreview.accordion.closed')) ?>
        </span>
    </div>
    <?php if ($content !== ''): ?>
