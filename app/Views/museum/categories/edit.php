...
<?= view('components/display/admin_page_header', [
    ...
    'title' => 'Museum.categories_edit',
]) ?>

<?php if (has_permission('catalog.category.delete')): ?>
    <form id="delete-item-form" method="post" action="<?= route_to('admin.museum.categories.delete', (string) ($item['id'] ?? '')) ?>" x-data @submit.prevent="$store.confirm.show('<?= esc(confirm_delete_message($item['name'] ?? $item['title'] ?? $item['id'] ?? null), 'js') ?>', () => $el.submit())">
        <?= csrf_field() ?>
    </form>
<?php endif; ?>

<form method="post" action="<?= route_to('admin.museum.categories.update', (string) ($item['id'] ?? '')) ?>" class="grid grid-cols-1 gap-6 lg:grid-cols-3">
    <?= csrf_field() ?>
                    ...
                    <?php foreach ($languages as $index => $lang): ?>
                        ...
                        <div x-show="isActive(<?= (int) $lang['id'] ?>)" class="space-y-4">
                            ...
                            <?= view('components/form/textarea', [
                                ...
                                'value' => old("translations.{$index}.short_description", $transValue['short_description'] ?? ''),
                                'rows' => 3,
                                'errors' => $errors ?? []
                            ]) ?>
                        </div>
    ...
    <aside class="space-y-6">
        <?= view('components/display/admin_actions_panel', [
            'content' => '<button type="submit" class="' . esc(action_button_class('primary'), 'attr') . ' w-full justify-center text-center py-2.5">' . esc(lang('App.save')) . '</button>'
                . (has_permission('catalog.category.delete') ? '<button type="submit" form="delete-item-form" class="' . esc(action_button_class('danger'), 'attr') . ' w-full justify-center text-center py-2.5 mt-2">' . esc(lang('App.delete')) . '</button>' : '')
